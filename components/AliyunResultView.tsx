...
function getSpeedColor(ms: number): string {
  if (ms <= 0) return '#e5e7eb'; // 灰色 = 无数据
  return getSpeedLevel(ms).color;
}

// ─── 主组件 ───────────────────────────────────────────────────────────────────

interface AliyunResultViewProps {
  data: {
    domain: string;
    checkedAt: number;
    rows: Array<{
...
type SortKey = 'nodeZh' | 'ip' | 'status' | 'totalTimeMs' | 'dnsTime' | 'connectTime' | 'firstByteTime';
type SortDir = 'asc' | 'desc';

export default function AliyunResultView({ data }: AliyunResultViewProps) {
  ...
  const filteredRows = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      ...
      if (sortKey === 'dnsTime' || sortKey === 'connectTime' || sortKey === 'firstByteTime') {
        va = parseFloat(va) || 0;
        vb = parseFloat(vb) || 0;
      }
  ...
  const ispList = useMemo(() => {
    const set = new Set(rows.map(r => r.isp));
      ...
      <div className="max-w-6xl mx-auto p-4 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5 pl-9">
              检测时间：{new Date(checkedAt).toLocaleString('zh-CN')} · 阿里云全国拨测
            </p>
          </div>
        ...
        {activeTab === 'ip' && (
          <div className="bg-card border border-border rounded p-4 space-y-3">
            ...
            {ipStats.map((stat, i) => {
              const pct = parseFloat(stat.percent);
              ...
              return (
                <div key={stat.ip} className="space-y-1">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-muted-foreground">{stat.count} 个节点 · {stat.percent}</span>
                  </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-muted/50 border-b border-border">
                    {([
                      ...
                      { key: 'dnsTime', label: 'DNS' },
                      { key: 'connectTime', label: '连接' },
                      { key: 'firstByteTime', label: '首包' },
                      { key: 'sslTime' as any, label: 'SSL' },
                ...
                <tbody>
                  {filteredRows.map((row, i) => {
                    const level = row.totalTimeMs > 0 ? getSpeedLevel(row.totalTimeMs) : null;
                      ...
                      <tr key={i} className={`border-b border-border/50 hover:bg-muted/30 transition-colors ${i % 2 === 0 ? '' : 'bg-muted/10'}`}>
                        ...
                        <td className="px-3 py-2">
                          <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs font-mono font-semibold ${
                            row.status === '200' ? 'bg-green-100 text-green-700' :
                            row.status ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-500'
                          }`}>
                            {row.status || '—'}
                          </span>
                        </td>
                        <td className="px-3 py-2 whitespace-nowrap">
                          {level ? (
                            <span className={`font-mono font-semibold ${level.text}`}>{row.totalTime}</span>
                          ) : (
                            <span className="text-muted-foreground">—</span>
                          )}
                        </td>
                        <td className="px-3 py-2 font-mono text-muted-foreground whitespace-nowrap">{row.dnsTime || '—'}</td>
                        <td className="px-3 py-2 font-mono text-muted-foreground whitespace-nowrap">{row.connectTime || '—'}</td>
                        <td className="px-3 py-2 font-mono text-muted-foreground whitespace-nowrap">{row.firstByteTime || '—'}</td>
                        <td className="px-3 py-2 font-mono text-muted-foreground whitespace-nowrap">{(row as any).sslTime || '—'}</td>
                      </tr>
                    );
                  })}
                </tbody>
